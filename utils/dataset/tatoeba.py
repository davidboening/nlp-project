...
class TatoebaDataset:
    ...
    def create_csv(force_override=False):
        output_path = (
            f"{DatasetConfig.DATASET_PROCESSED_DIR}/{DatasetConfig.TATOEBA_OUT_NAME}"
        )
        if not force_override and os.path.exists(output_path):
            print(
                DatasetConfig.SKIPPED_MSG_FORMAT.format(
                    file=DatasetConfig.TATOEBA_OUT_NAME
                )
            )
            return
        dataset = load_dataset(
            "tatoeba", lang1="en", lang2="ja", cache_dir=DatasetConfig.DATASET_RAW_DIR
        )
